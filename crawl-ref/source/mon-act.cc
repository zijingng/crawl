                                  ...
                                  const coord_def &targ)
{
    ...
    int target_count = count_adjacent_walls(targ, DNGN_SLIMY_WALL);
    target_count += count_adjacent_walls(targ, DNGN_ICY_WALL);

    // Entirely safe.
    if (!target_count)
        return false;

    int current_count = count_adjacent_walls(mon->pos(), DNGN_SLIMY_WALL);
    current_count += count_adjacent_walls(targ, DNGN_ICY_WALL);
